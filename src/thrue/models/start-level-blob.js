  ...
  methods: {
    $animate_startLevelBlob()
    {
      this.startLevelBlob.position.y =
        this.lerp(this.startLevelBlob.position.y,Math.sin(this.tcounter*0.03)*0.15+(
        (this.accs_length ||this.is_playing_test ) ? 25 : +0.6
         ),0.02)
      this.startLevelBlob.rotation.z = -Math.sin(this.tcounter*0.02)*0.35
      this.startLevelBlob.rotation.x = Math.sin(this.tcounter*0.05)*0.15
      this.startLevelBlob.rotation.y += 0.006-this.pointer.x*0.01
    },
